...
async function processTargetGroup(args, group, runtime = {}) {
  ...
  const chatIdText = Array.isArray(group.chatIds) && group.chatIds.length > 0
    ? ` | ${group.chatIds.length} destino(s) configurado(s)`
    : '';
  ...
  for (const target of group.targets) {
    ...
    if (webhookResult.skipped) {
      ...
      console.log(JSON.stringify(webhookPayload, null, args.pretty ? 2 : 0));
      continue;
    }
  ...
  if (allResults.length === 0) {
    ...
    logInfo(`${allResults.length} busca(s) processada(s); webhook desabilitado.`);
  }

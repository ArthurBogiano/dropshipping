...
async function collectProductsFromTarget(target, cookies, limit, options = {}) {
  ...
  const resendAfterMs = Math.max(0, Number(options.resendAfterMs || 0));
  const listingMaxPages = isCategoryLandingUrl(target) ? 1 : maxPages;
  const waitForAllowedWindow = typeof options.waitForAllowedWindow === 'function'
    ? options.waitForAllowedWindow
    : async () => {};
  ...
  if (isLikelyListingUrl(target)) {
    ...
    for (let pageNumber = 1; pageNumber <= listingMaxPages && products.length < limit; pageNumber += 1) {
      ...
      for (const productUrl of productUrls) {
        ...
        const alreadySent = db ? wasProductSentRecently(db, dedupeKey, resendAfterMs) : false;
